...
Database_manager::Database_manager(const char *database_uri) {
  ...
  extra::file_manager::create_folder(
      extra::file_manager::get_path_from_file_path(database_uri));
...
int Database_manager::set_scheme(std::list<s_table> &scheme, bool force_new) {
  ...
  for (it = scheme.begin(); it != scheme.end(); it++) {
    ...
    std::string columns =
        extra::string_join(it->columns.begin(), it->columns.end());
  ...
  std::string s_statements =
      extra::string_join(statements.begin(), statements.end(), "");
